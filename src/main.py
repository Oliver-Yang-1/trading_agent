...
# --- Temporary Messages Logging File Path ---
import os
# 假设 MESSAGES_LOG_FILE_MAIN 的定义与 api_utils.py 中的 MESSAGES_LOG_FILE 一致
# 确保这个路径是正确的，指向 data/temp_messages_log.txt
PROJECT_ROOT_FOR_LOG = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) # src 目录的父目录 (项目根目录)
MESSAGES_LOG_FILE_MAIN = os.path.join(PROJECT_ROOT_FOR_LOG, "data", "temp_messages_log.txt")
...
def run_hedge_fund(run_id: str, ticker: str, start_date: str, end_date: str, portfolio: dict, show_reasoning: bool = False, num_of_news: int = 5, show_summary: bool = False):
    ...
    try:
        ...
        data_dir = os.path.dirname(MESSAGES_LOG_FILE_MAIN)
        if not os.path.exists(data_dir):
            os.makedirs(data_dir, exist_ok=True)
        
        with open(MESSAGES_LOG_FILE_MAIN, "w", encoding="utf-8") as f:
            f.write("") # 写入空字符串以清空文件
        # logger.info(f"Cleared temporary messages log: {MESSAGES_LOG_FILE_MAIN}") # 可以保留这行日志用于确认
    except Exception as log_clear_e:
        # logger.error(f"Could not clear temporary messages log: {log_clear_e}") # 可以保留这行日志用于排错
        print(f"Warning: Could not clear temporary messages log: {log_clear_e}") # 或者用 print 简单提示
...
# Main analysis path (technical, fundamentals, sentiment, macro_news -> researchers -> ... -> macro_analyst)
...
workflow.add_edge("macro_news_agent", "researcher_bull_agent") # Added
...
workflow.add_edge("macro_news_agent", "researcher_bear_agent") # Added
