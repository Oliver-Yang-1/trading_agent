...
AGENT_MAP = {
    ...
    "fundamentals_agent": {"icon": "📝", "name": "Fundamentals"},
    "sentiment_agent": {"icon": "🔍", "name": "Sentiment"},
    "researcher_bull_agent": {"icon": "🐂", "name": "Bullish Research"},
    "researcher_bear_agent": {"icon": "🐻", "name": "Bearish Research"},
    "debate_room_agent": {"icon": "🗣️", "name": "Debate Room"},
    "risk_management_agent": {"icon": "⚠️", "name": "Risk Management"},
    "macro_analyst_agent": {"icon": "🌍", "name": "Macro Analyst"},
    "macro_news_agent": {"icon": "📰", "name": "Macro News"},
...
class StructuredTerminalOutput:
        ...
        if isinstance(data, dict):
            ...
            if agent_name == "portfolio_management_agent":
                ...
                if "action" in data:
                    ...
                    result.append(
                        f"{SYMBOLS['vertical']} Action: {action_icon} {action.upper() if action else ''}")

                if "quantity" in data:
                    quantity = data.get("quantity", 0)
                    result.append(f"{SYMBOLS['vertical']} Quantity: {quantity}")
                ...
                if "agent_signals" in data:
                    result.append(
                        f"{SYMBOLS['vertical']} {SYMBOLS['section_prefix']}Options of Analysts:")
                ...
                if "reasoning" in data:
                    ...
                    result.append(
                        f"{SYMBOLS['vertical']} {SYMBOLS['section_prefix']}Reasoning:")
                ...
                if "signal" in data:
                    ...
                    signal_icon = STATUS_ICONS.get(signal.lower(), "")
                    result.append(
                        f"{SYMBOLS['vertical']} Signal: {signal_icon} {signal}")
    ...
    def generate_output(self) -> str:
        ...
        title = f"Investment Analysis Report for Ticker: {ticker}"
