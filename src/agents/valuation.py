...
def valuation_agent(state: AgentState):
    ...
    data = state["data"]
    ticker = data["ticker"]  # Get ticker from state data
    ...
    # Calculate DCF gap
    if market_cap == 0:
        dcf_gap = 0  # Set to 0 when market cap is 0
        logger.warning(f"Market cap is 0 for {ticker}, setting DCF gap to 0")
    else:
        dcf_gap = (dcf_value - market_cap) / market_cap

    # Calculate owner earnings gap
    if market_cap == 0:
        owner_earnings_gap = 0
        logger.warning(f"Market cap is 0 for {ticker}, setting owner earnings gap to 0")
    else:
